...
function HeroSection({locale, c}: {locale: string; c: C}) {
  return (
    <section className="relative py-12 sm:py-16 lg:min-h-[70vh] lg:py-[75px] bg-gradient-to-br from-[#e6f7ff] to-white overflow-hidden">
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-0">
          <div className="w-full lg:w-1/2 relative z-10 lg:-mt-16 text-center lg:text-left">
            <EditableText contentKey="hero.title" locale={locale} value={c['hero.title']} tag="h1" className="text-3xl sm:text-4xl lg:text-5xl font-bold mb-4 sm:mb-6 text-[#004085] leading-tight" />
            <EditableText contentKey="hero.description" locale={locale} value={c['hero.description']} multiline tag="p" className="text-base sm:text-lg lg:text-xl leading-relaxed text-gray-700 lg:max-w-[90%]" />
          </div>
          <div className="w-full lg:w-1/2 relative lg:-mt-16">
            {/* Mobile/tablet: simple flex row of smaller circles */}
            <div className="flex justify-center items-center gap-3 sm:gap-4 lg:hidden">
              <EditableImage contentKey="hero.img1" locale={locale} src={c['hero.img1']} alt="" width={300} height={300}
                className="rounded-full border-4 border-white shadow-lg object-cover w-[28vw] h-[28vw] max-w-[180px] max-h-[180px] sm:w-40 sm:h-40 hover:scale-105 transition-transform duration-500" />
      ...
      <div className="container mx-auto px-4">
        <div className="flex flex-col lg:flex-row items-center gap-8 lg:gap-12">
          ...
          <div className="w-full lg:w-1/2">
            <EditableText contentKey="aboutSection.title" locale={locale} value={c['aboutSection.title']} tag="h2" className="text-2xl sm:text-3xl font-bold mb-4 sm:mb-6" />
            <EditableText contentKey="aboutSection.lead" locale={locale} value={c['aboutSection.lead']} multiline tag="p" className="text-base sm:text-lg leading-relaxed mb-4" />
            <EditableText contentKey="aboutSection.body" locale={locale} value={c['aboutSection.body']} multiline tag="p" className="mb-6" />
            <div className="flex flex-wrap gap-3">
              <Link href={`/${locale}/about`} className="bg-blue-600 text-white px-5 py-2.5 rounded-lg hover:bg-blue-700 transition-colors text-center">
                {t('learnMore')}
              </Link>
              <Link href={`/${locale}/membership`} className="border border-blue-600 text-blue-600 px-5 py-2.5 rounded-lg hover:bg-blue-50 transition-colors text-center">
                {t('joinAssociation')}
              </Link>
            </div>
          </div>
...
function MissionSection({locale, c}: {locale: string; c: C}) {
  ...
  return (
    <section className="py-12 sm:py-16 bg-gradient-to-br from-gray-50 to-gray-200 relative overflow-hidden">
      <div className="absolute top-[10%] left-[5%] opacity-5 text-[5rem] sm:text-[8rem] lg:text-[10rem] rotate-[15deg]">
        <i className="fas fa-bullseye" />
      </div>
      <div className="absolute bottom-[10%] right-[5%] opacity-5 text-[4rem] sm:text-[6rem] lg:text-[8rem] -rotate-[20deg]">
        <i className="fas fa-hands-helping" />
      </div>

      <div className="container mx-auto px-4">
        <EditableText contentKey="mission.sectionTitle" locale={locale} value={c['mission.sectionTitle']} tag="h2" className="text-3xl sm:text-4xl font-bold text-center text-[#2c3e50] mb-8 sm:mb-10 section-title-underline centered" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 sm:gap-8">
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-10 transition-all duration-400 hover:-translate-y-4 hover:shadow-xl relative overflow-hidden border-b-4 border-transparent before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-[5px] before:bg-[#3498db] before:scale-x-0 before:origin-left before:transition-transform before:duration-400 hover:before:scale-x-100">
            <EditableText contentKey="mission.missionTitle" locale={locale} value={c['mission.missionTitle']} tag="h3" className="text-xl sm:text-2xl font-bold mb-4 sm:mb-5 text-[#2c3e50] text-center pb-4 sm:pb-8" />
            <EditableText contentKey="mission.missionText" locale={locale} value={c['mission.missionText']} multiline tag="p" className="text-base sm:text-lg leading-relaxed text-[#34495e]" />
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-10 transition-all duration-400 hover:-translate-y-4 hover:shadow-xl relative overflow-hidden border-b-4 border-transparent before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-[5px] before:bg-[#e74c3c] before:scale-x-0 before:origin-left before:transition-transform before:duration-400 hover:before:scale-x-100">
            <EditableText contentKey="mission.valuesTitle" locale={locale} value={c['mission.valuesTitle']} tag="h3" className="text-xl sm:text-2xl font-bold mb-4 sm:mb-5 text-[#2c3e50] text-center pb-4 sm:pb-8" />
          ...
          <div className="bg-white rounded-2xl shadow-lg p-6 sm:p-8 lg:p-10 transition-all duration-400 hover:-translate-y-4 hover:shadow-xl relative overflow-hidden border-b-4 border-transparent before:content-[''] before:absolute before:top-0 before:left-0 before:w-full before:h-[5px] before:bg-[#27ae60] before:scale-x-0 before:origin-left before:transition-transform before:duration-400 hover:before:scale-x-100">
            <EditableText contentKey="mission.goalsTitle" locale={locale} value={c['mission.goalsTitle']} tag="h3" className="text-xl sm:text-2xl font-bold mb-4 sm:mb-5 text-[#2c3e50] text-center pb-4 sm:pb-8" />
            <EditableText contentKey="mission.goalsText" locale={locale} value={c['mission.goalsText']} multiline tag="p" className="text-base sm:text-lg leading-relaxed text-[#34495e]" />
          </div>
        </div>
      </div>
      <div className="text-center mt-8 mb-4 sm:mb-10">
        <Link href={`/${locale}/about`} className="inline-block border border-blue-600 text-blue-600 px-6 py-2.5 rounded-lg hover:bg-blue-50 transition-colors">
          {t('learnMore')}
        </Link>
      </div>
    </section>
    ...
    <section className="py-12 bg-white">
      <div className="container mx-auto px-4">
        <div className="text-center mb-10 sm:mb-16">
          <EditableText contentKey="membership.sectionTitle" locale={locale} value={c['membership.sectionTitle']} tag="h2" className="text-3xl sm:text-4xl font-bold mb-4 sm:mb-5 text-[#2c3e50]" />
          <EditableText contentKey="membership.subtitle" locale={locale} value={c['membership.subtitle']} multiline tag="p" className="text-base sm:text-lg text-gray-500 max-w-[700px] mx-auto" />
        </div>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 sm:gap-8">
          {benefits.map((b, i) => (
            <div key={i} className="bg-gray-100 rounded-2xl flex flex-col transition-all duration-400 hover:-translate-y-2.5 hover:shadow-xl">
              <div className="text-5xl mx-auto mt-8 mb-5">{b.icon}</div>
              <div className="px-6 sm:px-8 pb-8 text-center flex-grow flex flex-col">
                <EditableText
                  contentKey={`membership.benefit.${i}.title`}
                  locale={locale}
                  value={c[`membership.benefit.${i}.title`] ?? b.title}
                  tag="h3"
                  className="text-lg sm:text-xl font-bold mb-4 text-[#2c3e50]"
                />
                <EditableText
                  ...
                  multiline
                  tag="p"
                  className="text-base sm:text-lg leading-relaxed text-gray-600 mb-5"
                />
                <EditableText
