...
export default function MembershipPage() {
  ...
  const btnColors = ['btn-outline-primary', 'btn-outline-success', 'btn-outline-info'];
    ...
    <>
      ...
      <div className="mt-4 mx-4 w-full max-w-[200vh] bg-white rounded-2xl overflow-hidden shadow-2xl">
        <div className="bg-gradient-to-r from-[#1a6aa2] to-[#2a5298] p-10 text-center">
          <div className="text-2xl font-bold text-white mb-2 tracking-wide">{t('associationName')}</div>
          <h1 className="text-4xl font-extrabold text-white mb-4 [text-shadow:0_2px_10px_rgba(0,0,0,0.2)]">
            {t('title')}
          </h1>
          <p className="text-lg text-white/90 max-w-[700px] mx-auto leading-relaxed">{t('subtitle')}</p>
        </div>
          ...
          {benefits.map((b, i) => (
            <div key={i} className="flex flex-col md:flex-row items-center md:items-start gap-6 pb-8 border-b border-gray-200 last:border-0 last:pb-0">
              <div className="min-w-[70px] h-[70px] bg-[#e6f0fa] rounded-full flex items-center justify-center flex-shrink-0">
                <i className={`fas ${benefitIcons[i]} text-2xl text-[#1a6aa2]`} />
              </div>
              <div>
                <h3 className="text-xl font-semibold text-[#2a5298] mb-3">{b.title}</h3>
      ...
      <div className="container mx-auto px-4 mt-24 mb-12">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold text-gray-800">{t('howToJoinTitle')}</h1>
          <p className="text-gray-500 mt-2">{t('howToJoinSubtitle')}</p>
        </div>
